...
export default function SparkleButton({ text, onClick, disabled, className, type = "button" }: { 
  text: string, 
  onClick?: () => void, 
  disabled?: boolean, 
  className?: string,
  ...
  return (
    <div className={`sp ${className || ''}`}>
      ...
      >
        ...
        <span className="text">{text || "Generate Site"}</span>
      </button>
      <div className="bodydrop"></div>
      <span aria-hidden="true" className="particle-pen">
        {[...Array(10)].map((_, i) => (
          <svg key={i} className="particle" style={{
            ...
            "--origin-x": `${Math.random() > 0.5 ? Math.random() * 800 + 200 : Math.random() * -800 - 200}%`,
            "--origin-y": `${Math.random() > 0.5 ? Math.random() * 800 + 200 : Math.random() * -800 - 200}%`
          } as React.CSSProperties} viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
